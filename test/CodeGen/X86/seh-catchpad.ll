...
__try.cont.12:                                    ; preds = %invoke.cont.3, %entry, %__except.9
  ...
  %cs1 = catchswitch none, unwind label %catch.dispatch.7 [label %__except]

__except:                                         ; preds = %catch.dispatch
  %cp1 = catchpad %cs1 [i8* null]
  catchret %cp1 to label %__except.2
...
invoke.cont.6:                                    ; preds = %ehcleanup
  cleanupret %cp2 unwind label %catch.dispatch.7

catch.dispatch.7:
  %cs2 = catchswitch none, unwind to caller [label %__except.ret]

__except.ret:                                     ; preds = %catch.dispatch.7
  %cp3 = catchpad %cs2 [i8* bitcast (i32 (i8*, i8*)* @"\01?filt$0@0@main@@" to i8*)]
  catchret %cp3 to label %__except.9
}
