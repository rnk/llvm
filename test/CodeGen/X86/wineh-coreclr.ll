...
catch1.pad:
  %cs1 = catchswitch none, unwind label %finally.pad [label %catch1.body, label %catch2.body]
catch1.body:
  %catch1 = catchpad %cs1 [i32 1]
...
; CHECK: .seh_proc [[L_fault:[^ ]+]]
  %fault = cleanuppad none [i32 undef]
; CHECK: .seh_stackalloc [[FuncletFrameSize:[0-9]+]]
;                        ^ all funclets use the same frame size
; CHECK: movq [[PSPSymOffset]](%rcx), %rcx
;                              ^ establisher frame pointer passed in rcx
...
fault.ret:
  cleanupret %fault unwind label %finally.pad
catch2.ret:
