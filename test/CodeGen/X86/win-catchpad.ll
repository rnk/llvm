...
entry:
  ...
  %cs = catchswitch none, unwind to caller [label %handler1, label %handler2]

handler1:
  %h1 = catchpad %cs [%rtti.TypeDescriptor2* @"\01??_R0H@8", i32 0, i32* %e.addr]
  %e = load i32, i32* %e.addr
  call void @f(i32 %e, i32* %local)
  catchret %h1 to label %try.cont

handler2:
  %h2 = catchpad %cs [i8* null, i32 64, i8* null]
  call void @f(i32 3, i32* %local)
  catchret %h2 to label %try.cont
...
entry:
  ...
  %cp1 = catchpad %cs1 [i8* null, i32 64, i8* null]
  br label %loop
...
loop:
  ...
  catchret %cp1 to label %try.cont
