...
static cl::opt<bool>
MergeNoReturnBBs("merge-noreturn-bbs", cl::Hidden, cl::init(false),
                cl::desc("Whether to tail merge noreturn basic blocks"));
...
tailMergeNoReturnBlocksIfProfitable(BasicBlock *BB1, BasicBlock *BB2,
  ...
  for (; I1 != E1 && I2 != E2;
    ...
    if (!I1->isSameOperationAs(&*I2)) {
      DEBUG(dbgs() << "noreturn tail merge of " << BB1->getName() << " and "
                   << BB2->getName()
    ...
    for (Value *Op1 : I1->operands()) {
      ...
      if (!(Op1 == Op2 ||
        ...
        DEBUG(dbgs() << "noreturn tail merge of " << BB1->getName() << " and "
                     << BB2->getName() << " failed due to differing operands:\n"
                     << *Op1 << '\n'
                     << *Op2 << '\n');
        return false;
      }
  ...
  DEBUG(dbgs() << "Tail merging " << BB1->getName() << " with "
               << BB2->getName() << '\n');
  ...
  // llvm.dbg.value intrinsics. Leave the debug locations of the BB1.
...
///     call void @abort()
///     unreachable
static bool tailMergeNoReturnBlocks(Function &F) {
  // Collect all non-empty noreturn blocks and hash them into buckets.
  ...
  for (auto &HashBucket : Buckets) {
    ...
    while (Bucket.size() > 1) {
      DEBUG({
        dbgs() << "tailMergeNoReturnBlocks: considering";
        for (auto *BB : Bucket)
          dbgs() << ' ' << BB->getName();
        dbgs() << '\n';
      });
...
static bool simplifyFunctionCFG(Function &F, const TargetTransformInfo &TTI,
  ...
  if (MergeNoReturnBBs)
    EverChanged |= tailMergeNoReturnBlocks(F);
