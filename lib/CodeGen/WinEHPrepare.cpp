...
static void calculateSEHStateNumbers(WinEHFuncInfo &FuncInfo,
  ...
  if (auto *CatchSwitch = dyn_cast<CatchSwitchInst>(FirstNonPHI)) {
    ...
    const auto *CatchPad = cast<CatchPadInst>(
        cast<BasicBlock>(CatchSwitch->handler_begin())->getFirstNonPHI());
...
static bool isTopLevelPad(const Instruction *EHPad) {
  if (auto *CatchSwitch = dyn_cast<CatchSwitchInst>(EHPad))
    return CatchSwitch->inOutermostScope();
  if (auto *CleanupPad = dyn_cast<CleanupPadInst>(EHPad))
    return CleanupPad->inOutermostScope();
  return false;
}
...
void WinEHPrepare::replaceTerminatePadWithCleanup(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    auto *CPI =
        CleanupPadInst::Create(TPI->getOuterScope(), {},
...
void llvm::calculateCatchReturnSuccessorColors(const Function *Fn,
  ...
  for (const BasicBlock &BB : *Fn) {
    ...
    if (isa<ConstantTokenNone>(OuterScope))
      ...
      Color = cast<Instruction>(OuterScope)->getParent();
